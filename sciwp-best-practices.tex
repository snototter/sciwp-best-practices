...
% 
% Change history:
% * 10/2021-11/2021 hp: v1.6 Public release
...
\label{sec-intro}
Scientific communication -- especially writing, but also presenting -- is a craft with a steep learning curve, as every ``fresh'' PhD student is about to learn.
The goal of this document is to accompany students on this ``climb'' and provide you helpful tipps \& recommendations for writing your first scientific papers and presenting your work.

\todo{refactor}
We both taught this craft for several years at Graz University of Technology...
This document accompanies our seminar \emph{"Writing Scientific Papers"} (German: \emph{"Verfassen wissenschaftlicher Arbeiten"}) where students should learn (1)~how to write scientific papers and (2)~how to prepare and give a scientific presentation. 

From our experience over the years, we summarized typical errors, so you can avoid them from the beginning.
This guideline document presents the most important \goodstyle{dos} and \badstyle{dont's} to help you get started with scientific writing/presenting.
To this end, you should be aware of the following topics:
% An additional goal is to point out typical errors, to avoid them from the beginning. In fact, three out of four comments are the same for all students. In this way, we collected such typical error cases (from the experience of former courses) and additionally give examples for \goodstyle{dos} and \badstyle{dont's}, with special emphasis on typesetting in \LaTeX. Indeed, this is a living document, extended and adapted permanently, but the main points are already summarized. In particular, we summarize the following topics:
\begin{itemize}
...
\end{itemize}
% Please note that the presented information is \textbf{mandatory for all submissions throughout our seminar}.
% In fact, these \textbf{guidelines and examples will be considered to be known and thus, also considered for grading!}
% Beside this course, these guidelines will also help you write any other scientific document (for example, bachelor's or master's thesis, technical reports, \emph{etc.}) and you are highly welcome to use and refer to this guideline later on.
\todo{note on typesetting with latex}

% \subsubsection*{Typesetting:}
% In general, you can use any typesetting software to prepare your submissions (including office applications such as Microsoft Word or LibreOffice), as long as you adhere to the defined layout requirements.
% However, we highly recommend using \LaTeX, as it simplifies layouting scientific texts a lot.
...
% 
% To help you get started with \LaTeX, this document provides several typesetting examples. You can readily use these examples with the provided template to prepare your submissions. 
% This, however, is not a step-by-step \LaTeX~tutorial.
% If you have never used \LaTeX~before, you may want to check out a beginner's tutorial, for example, \url{https://latex.tugraz.at} and refer to Q\&A sites, such as \url{https://tex.stackexchange.com}.

\todo{note on spellchecking \& proofreading}
...
\begin{goodexample}[Correct Citation of a Single Paper]
  \begin{NoHyper}
    \input{examples/single_citation_good.tex}
...
\begin{goodexample}[Correct Citation of Multiple Papers]
  \begin{NoHyper}
    \input{examples/multiple_citations_good.tex}
...
\begin{badexample}[Incorrect Citation of Multiple Papers]
  \begin{NoHyper}
    \input{examples/multiple_citations_bad.tex}
  \end{NoHyper}
\end{badexample}
...
% \item Correctness: \textbf{Never copy references without double-checking} -- not even from Google Scholar, ResearchGate, etc. Always check references yourself!

% \item Consistency: Use the same style, the same abbreviations, the same detail
% level, the same name/style for conferences and journals.


% \item Completeness: All BibTeX-entries must contain the following fields:
% %
% \vspace{-0.25cm}
% %
% \begin{itemize}
% \item Authors
% \item Title
% \item Source (book, journal, conference, etc.)
% \item Issue, number (for journals and technical reports)
% \item Page numbers (for journals)
% \item Year
% \end{itemize}

\item Each reference in your bibliography \textbf{must be cited at least once} within the text.
% In contrast to other publications, in scientific works, each reference in the bibliography \textbf{must be cited at least once} in the text.
...
\vfill
\subsection{Mandatory Bibliography Fields}
\label{sec-bibtex-entries}

As mentioned above, depending on the publication type you want to cite (\emph{e.g.} conference papers, books, \emph{etc.}), there are different \textbf{mandatory fields} you must provide in your bibliography. Below, you'll find examples for (almost) all publication types you'll need.
For each type, we list the mandatory fields you have to provide in your bibliography (such as \texttt{title}, \texttt{author}, \emph{etc.}) within the corresponding exemplary BibTeX entry.
...
\bibtexample{Conference Paper}{examples/bibtex_proceedings.tex}{%
  \begin{itemize}
    \item[{[17]}] J. Doe and J.~J. Smith. A Solution to Foo. In \textit{Proceedings of the IEEE Conference on Scientific Writing}, 2014.
  \end{itemize}%
}{%
  \begin{itemize}
    \item You have to \textbf{explicitly write} ``Proceedings of the'' or ``Proc. of the'' inside the \texttt{booktitle} field.
    ...
    \item Ensure that you write the conference names correctly ordered -- if an online library uses lexicographic entries, \emph{e.g.} conference name similar to \emph{``\badstyle{Scientific Writing, Int. Conf. on}''}, you have to use \texttt{booktitle = \{Proceedings of the International Conference on Scientific Writing\}} instead.
    \item Ordinal numbers should not be included in the conference name, \emph{e.g.} instead of \emph{``\badstyle{23\textsuperscript{rd}} International Conference on Frobnication''} use \texttt{booktitle = \{Proceedings of the International Conference on Frobnication\}}.
  \end{itemize}%
}
...
\bibtexample{Journal Article}{examples/bibtex_journal.tex}{%
  ...
  \begin{itemize}
   \item Some journals only publish \emph{volumes} -- \textbf{always double-check} if there is a \emph{number/issue}.
   % \item This example also shows how to correctly typeset \emph{diacritics} or \emph{umlauts}, both in LaTeX and BibTeX, \emph{e.g.} \texttt{\textbackslash{v}\{r\}} produces \v{r}, \texttt{\textbackslash"A} produces \"A, \texttt{\textbackslash'a} produces \'a, \emph{etc.}
   \item Use a \textbf{hyphen} (double dash) for the page range, \emph{i.e.} \texttt{pages = \{1409--1422\}}. A single dash should only be used for compound words, \emph{e.g.} ``state-of-the-art''.
  \end{itemize}
}
\vfill
...
\bibtexample{Book Chapter}{examples/bibtex_bookchapter.tex}{%
  \begin{itemize}
    \item[{[49]}] C.~M. Bishop and J. Lasserre. \textit{Bayesian Statistics}, volume 8, chapter \textit{Generative or Discriminative? Getting the Best of Both Worlds}, pages 3--24. Oxford University Press, 2007.
  \end{itemize}%
}{%
  \begin{itemize}
   \item Be aware that in this case the \emph{title} field is the \textbf{title of the
   book}, whereas the \emph{chapter} field is the name of the \textbf{actual
   contribution!}
  \end{itemize}
}
...
\bibtexample{Thesis}{examples/bibtex_thesis.tex}{%
  ...
  \end{itemize}%
    ...
    \item If you cite a master's thesis, use the BibTeX entry \texttt{@mscthesis} instead.
  \end{itemize}}
...
\bibtexample{Papers found on arXiv}{examples/bibtex_arxiv.tex}{%
  ...
  \end{itemize}%
    ...
    \item \textbf{Double-check any arXiv content} -- has the paper already been published at a conference/journal?
  \end{itemize}}
% TODO Peter: bitte aendern, falls dir mein arXiv Stil nicht passt

% \vfill
%% URLs
\bibtexample{URLs}{examples/bibtex_url.tex}{%
  \begin{itemize}
    \item[{[13]}] J.-Y. Bouguet. Camera Calibration Toolbox for Matlab. \texttt{http://www. vision.caltech.edu/bouguetj/calib\_doc/index.html}, 2010. Online; accessed July 26, 2017.
  \end{itemize}%
}{%
\begin{itemize}
  \item Use URLs \textbf{only if you really have to}, \emph{e.g.} referring to a software toolbox.
...
\vspace{-0.6cm}
\bibtexampleplain{BibTeX Special Formating Rules}{
\begin{itemize}[leftmargin=1em]
  \item The extra curly braces (\emph{e.g.} \texttt{\{F\}oo}) within the \texttt{title} ensure that the capitalization won't be changed by BibTeX.

  \item To ensure a correct typesetting of \emph{diacritics} or \emph{umlauts}, use the corresponding \LaTeX-commands: \emph{e.g.} \texttt{\textbackslash{v}\{r\}} produces \v{r}, \texttt{\textbackslash"A} produces \"A, \texttt{\textbackslash'a} produces \'a, \emph{etc.} See also Section~\ref{sec-spec:tex} on how to typeset umlauts and diacritics in \LaTeX.
...
% \item Additionally be aware, that most published papers hold a copyright -- so by simply copying an illustration, you can easily violate copyright law (even if you cited the source correctly).
...
\begin{itemize}
 ...
 \item Each table \textbf{must be referenced} at least once within your text, \emph{e.g.}~``see Table~\nohref{tab-example1}''; ``this is summarized in Table~\nohref{tab-example1}'', \emph{etc.}
 
 \item A numbered table is a \emph{proper noun}, thus never write ``\badstyle{see table~\nohref{tab-example1}}'', only ``\goodstyle{see Table~\nohref{tab-example1}}'' is correct. Never ever use the (incorrect) abbreviation ``\badstyle{Tab.~\nohref{tab-example1}}''.
...
\begin{texexample}[Typesetting a Table]
  ...
  \begin{table}[H]
    \centering
    \caption{A good (self-explanatory) caption would explain that higher ``frobnability'' scores indicate better performance of a method.}
    \label{tab-example1}
    \begin{tabular}{l|c}
...
\begin{itemize}
 ...
 \item Pay attention to image credits and avoid copyright infringement (see also Section~\ref{sec-plagiarism})! Explicitly state if you \textbf{copied} a figure (write: \emph{``\goodstyle{Image taken from [X]}''}) or \textbf{modified} a figure (\emph{``\goodstyle{Image adapted from [X]}''}) within its caption.
 ...
 \item A numbered figure is a \emph{proper noun}, thus never write ``\badstyle{see figure~\nohref{fig-example1}}'', only ``\goodstyle{see Figure~\nohref{fig-example1}}'' or ``\goodstyle{see Fig.~\nohref{fig-example1}}'' is correct.
...
\begin{texexample}[Typesetting Figures]
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.35\textwidth]{examples/tug_logo}
    ...
    Note that ``\badstyle{Illustration of the university logo~\nohrefcite{smith14}}'' is incorrect -- this citation is ambiguous and does not clearly state that the image was copied from somewhere else.}
    \label{fig-example1}
  \end{figure}
  \vspace{-0.3em}
  \begin{figure}[H]
    \centering
    \subfloat[The ICG institute.]{\makebox[0.4\textwidth][c]{\includegraphics[width=0.3\textwidth]{examples/icg_logo}}}\hspace{0.5cm}
    \subfloat[The LRS workgroup.]{\makebox[0.4\textwidth][c]{\includegraphics[width=0.3\textwidth]{examples/lrs_logo}}}
...
%   \vspace{0.5cm}
  \item\textbf{Corresponding reference within the text:}\\
  Our logos are shown in Figures~\ref{fig-example1} and~\ref{fig-example2}.
  \end{itemize}
  \vspace{0.5cm}
...
% For mathematical writing several simple rules have to be followed, where the
% most important are given below. 
% For more details we would like to refer to \citefurther{book:MLA09}, \citefurther{book:CMS17}, and \citefurther{book:Vivaldi14},.
...
%(TOO MUCH INFORMATION FOR STUDENTS) Nevertheless, \LaTeX~treats equations as floating objects. Thus, to avoid unusual positions either do not use empty lines or use '\textbf{\%}' to glue the text to the equation. Otherwise, you should use the \lstinline[language={[LaTeX]TeX},basicstyle=\bfseries]{\noindent} command, to avoid that the following text is (incorrectly) indented.
...
\item When referencing an equation, put its number into parentheses, \emph{e.g.} ``\goodstyle{see Equation~(1)}'' or ``\goodstyle{see Eq.~(1)}''.  \LaTeX~takes care of the parentheses if you use the \verb,\eqref{}, command.  

\item A numbered equation is a \emph{proper noun}, thus ``\badstyle{in equation~(1)}'' is wrong and must be replaced by ``\goodstyle{in Equation~(1)}''.
...
\begin{texexample}[Math -- Inline vs. Numbered]
  ...
  \input{examples/math_text0.tex}
  
  \inlatex{examples/math_text0.tex}
\end{texexample}

% One of the major benefits of \LaTeX~is its simple means to typeset mathematics.
% You can write inline equations, such as $E = m c^2$, or place them in a separate environment, just like
% \begin{equation}
%  i^\star = \underset{i}{\arg\min} \sum_{j=i}^{N} \langle\mathbf{x}_i, \mathbf{x}_j \rangle. \label{eq1}
% \end{equation}
% Equations can also be labeled to refer to it via \verb,\eqref{},, see Equation~\eqref{eq1}.
% Use the \texttt{align} environment to typeset multi-line equations like this
% \begin{align}
%   \int_0^\infty e^{-\alpha x^2} \mathrm{d}x   =   \frac12\sqrt{\int_{-\infty}^\infty e^{-\alpha x^2}} \mathrm{d}x\int_{-\infty}^\infty e^{-\alpha y^2}\mathrm{d}y\\ 
...
\begin{texexample}[Math -- Matrices]
  \input{examples/math_projection.tex}
...
\begin{texexample}[Math -- Multiline Equations]
  \input{examples/math_multiline.tex}
...
\begin{badexample}[Plagiarism -- Copying Text]
  \label{ex-plagiat1}
  \begin{figure}[H]
    \centering%
    {%
...
\begin{badexample}[Plagiarism -- Paraphrasing Text]
  \begin{figure}[H]
    \centering%
    {%
      \setlength{\fboxsep}{0pt}%
...
\begin{badexample}[Plagiarism -- Translating Text]
  \begin{figure}[H]
    \centering%
    {%
      \setlength{\fboxsep}{0pt}%
...
\begin{goodexample}[Slides -- Image Sources]
  ...
  \begin{itemize}[leftmargin=6pt]
    ...
    \item Avoid full URLs (e.g. if you copied illustrations) which would clutter your slides. \textbf{Shorten web links and use hyperlinks} to the full URL instead. For example, the top-left image source {\goodstyle{[www.icg.tugraz.at]}} is a hyperlink pointing to {\tiny\texttt{https://www.tugraz.at/fileadmin/\_processed\_/7/4/hololens\_header\_960.jpg}}
...
\begin{goodexample}[Slides -- Single Reference]
  \begin{figure}[H]
    \centering%
    {%
      \setlength{\fboxsep}{0pt}%
...
%   Additionally, ``we'' in papers expresses modesty of the author.
...
An exception is in the \emph{sentence case} to increase the readability: ``\goodstyle{How to give a good presentation?}'' or ``\goodstyle{An image is worth a thousand words.}'' 

% When in doubt, capitalize all words (in headings and the title), except for \emph{a}, \emph{an}, \emph{the}, \emph{by}, \emph{for}, \emph{in}, \emph{of}, \emph{on}, \emph{to}, \emph{up}, \emph{and}, \emph{as}, \emph{but}, \emph{or}, and \emph{nor}.
...
\begin{itemize}
  ...
  \item Use scientific diction/professional wording. For example, compare ``\goodstyle{a large dataset/a large amount of data}'' \emph{versus} ``\badstyle{a big set of data}''.
...
%   \item Provided templates have to be used
% Das muss man den Studenten echt aufschreiben? WTF!
...
\end{texexample}
 ...
 \begin{texexample}[\LaTeX -- Special Characters]
   \input{examples/specialchars.tex}
...
% \pagebreak 
 ...
 \begin{goodexample}[\LaTeX -- Correct Usage of Protected Spaces]
   \begin{NoHyper}
     \input{examples/protected_spaces.tex}
 ...
 \begin{badexample}[\LaTeX -- Incorrect Usage of Protected Spaces]
   \begin{NoHyper}
%    \input{examples/protected_spaces_incorrect.tex}
